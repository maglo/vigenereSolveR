min.keylength <- 2
max.keylength <- 13
ctext <- "0102030405060708090A0B0C0D0E0F"
ctext <- ""
num.chars <- nchar(ctext) #determine number of characters
#num.chars
num.bytes <- num.chars / 2 #two chars describe a byte
starts <- seq(1, num.chars, by=2) #positions for chopping ctext up in 2 char vector
#starts

#chop it up and pad with hex prefix
chopped <- sapply(starts, function(ii){  
    substr(ctext, ii, ii+1)
})
chopped <- paste("0x", chopped, sep = "")
#str(chopped)

crypto <- data.frame(chopped)
#str(crypto)
#head(crypto)
#crypto

# create permutations of all logical vectors for ananlysis of every nth byte
for (keylength in min.keylength:max.keylength) {
    for (series in 1:(keylength)) {
        compare <- keylength - series
        tmp <-  ((1:(num.bytes + 1) + compare) %% keylength == 0)[0:num.bytes]
        crypto[,paste("kl",keylength,"s",series,sep="")] <- tmp
    }
}


#create a keymask for each keylenght
keymask <- crypto[,2:length(crypto)]
#keymask

#apply the keymask to the ctext so that every nth char is gobbled
subsets <- apply(keymask, 2, function(ii){
    subset(crypto$chopped, subset=ii)
})
#all

# tabulate all subsets

kldata <- as.list(NULL)
for(i in 1:length(subsets)){
    kldata[[names(subsets)[i]]] <- as.vector(prop.table(table(subsets[i])))
}

#str(kldata)

#number_of_columns <- 0
#results <- data.frame(t(rep(NA,number_of_columns)))
#names(results) <- c()
results <- NULL

for (i in 1:length(kldata)){
    tmp <- unlist(kldata[i])
    name <- names(kldata[i])
    fraction <- sum(tmp[1:length(tmp)] ** 2)
    data <- c(name, fraction)
    results <- cbind(results,data)
}

results <- t(results)
#results

results <- data.frame(results, row.names=results[,1])
names(results) <- c('KeylengthID', 'Sum')

#str(results)
#summary(results)
#head(results)

order.keylength <- order(results[,2], decreasing = TRUE)
#order.keylength

results <- results[order.keylength,]

sorted <- NULL
for (keylength in min.keylength:max.keylength){
    pattern <- paste("kl", keylength, "s.*", sep="")
    #print(pattern)
    subsetmask <- grep(pattern, results$KeylengthID)
    subset <- results[subsetmask,]
    #print(typeof(subset))
    #print (subset)
    #str(subset)
    tmp <- as.vector(subset$Sum)
    #print(tmp)
    #str(tmp)
    tmp <- as.numeric(tmp)
    #print(tmp)
    #str(tmp)
    #print(typeof(tmp))
    #print(mean(tmp))
    sorted[paste(keylength, sep="")] <- mean(tmp)
}
sorted <- t(t(sorted))
#sorted

order.keylength <- order(sorted, decreasing = TRUE)
#order.keylength
sorted <- t(t(sorted[order.keylength,]))
#sorted

probable.keylength <- dimnames(sorted)[[1]][1]

print(paste("The probable keylength is ", probable.keylength, sep = ""))
#barplot(sorted[,1])

